...
int hci_setup_ext_adv_instance_sync(struct hci_dev *hdev, u8 instance)
{
	...
	/* adv interval: 100 * 0.625 ms */
	hci_cpu_to_le24(133, cp.min_interval);
	hci_cpu_to_le24(133, cp.max_interval);
...
static int hci_le_set_event_mask_sync(struct hci_dev *hdev)
{
	...
	/* If the controller supports the LE Periodic Advertising
	 * command, enable the corresponding event.
	 */
	if (per_adv_capable(hdev)) {
		events[1] |= 0x20;	/* LE Periodic Advertising Sync Established, ping_yan add */
		events[1] |= 0x40;	/* LE Periodic Advertising Report, ping_yan add */
	}
	...
	if (bis_capable(hdev)) {
		...
		events[4] |= 0x02;	/* LE BIGInfo Advertising Report event, ping_yan add */
	}
...
int hci_dev_open_sync(struct hci_dev *hdev)
{
	...
	while((ret = hci_dev_init_sync(hdev)) < 0); //kx
	if (!ret) {
		hci_dev_hold(hdev);
		hci_dev_set_flag(hdev, HCI_RPA_EXPIRED);
