			       ...
			       char __user *optval, unsigned int optlen)
{
	...
	case BT_ISO_BASE:
		...
		if (copy_from_user((char *)(iso_pi(sk)->base), optval, len)) {
			err = -EFAULT;
			break;
		}
...
void iso_recv(struct hci_conn *hcon, struct sk_buff *skb, u16 flags)
{
	...
	case ISO_SINGLE:
		...
		/* ping_yan revised because parse hdr error when ts == 1 */
		if (ts) {
			...
			hdr_slen = ((struct hci_iso_ts_data_hdr *)hdr)->slen;
		} else {
			...
			hdr_slen = ((struct hci_iso_data_hdr *)hdr)->slen;
		}

		len    = __le16_to_cpu(hdr_slen);
...
int iso_init(void)
{
	...
	iso_inited = true; /* ping_yan moved here since bt_debugfs is error. */
