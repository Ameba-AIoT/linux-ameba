                                      ...
                                      struct drm_atomic_state *state)
{
	...
	if( CHECK_IS_NULL(kcrtc) || !kcrtc->enable)
		return;
                                    ...
                                    struct drm_atomic_state *state)
{
	...
	if( CHECK_IS_NULL(new_plane_state) )
		return -EINVAL;
                                      ...
                                      struct drm_atomic_state *state)
{
	...
	/* Sets the FB address */
	paddr = (u32)drm_fb_dma_get_gem_addr(fb, new_state, 0);
	vaddr = phys_to_virt(paddr);

	if (!vaddr) {
		pr_err("Failed to get virtual address");
		return;
	}
	...
	if (new_state->crtc_x >= display_width || new_state->crtc_y >= display_height) {
		DRM_WARN("[error happen] %s-%d [start[%d-%d][%d-%d]\n", __func__, __LINE__,
				 new_state->crtc_x, new_state->crtc_y, display_width, display_height);
		return;
	}

	/* display from 1 ~ MIPI_DISPLAY_WIDTH */
	dest_x = new_state->crtc_x ; //((new_state->crtc_x==0)?(1):(new_state->crtc_x));
	dest_y = new_state->crtc_y ; //((new_state->crtc_y==0)?(1):(new_state->crtc_y));
	...
	dest_w = ((new_state->crtc_w > src_w) ? (src_w) : (new_state->crtc_w));
	if (dest_x + dest_w > display_width) {
		dest_w = display_width - dest_x;
	}
	dest_h = ((new_state->crtc_h > src_h) ? (src_h) : (new_state->crtc_h));
	 ...
	 * new_state->src_x!=0 or new_state->src_y !=0 , it is not from the first pixel
	 */
	if (dest_w != fb->width || src_x || src_y) {
		...
		for (y = 0; y < dest_h ; ++y) {
			memcpy((uint8_t *)sec_layer_info[idx].sec_vaddr + (y * dest_w * bpp),
					(uint8_t *)vaddr + ((y + src_y) * (fb->width) * bpp) + (bpp * src_x),
					dest_w * bpp);
		}
	...
	if (sec_layer_info[idx].use_sec_buffer) {
		...
		ameba_lcdc_layer_imgaddress(lcdc_tmp, idx, (u32)(paddr));
	}
	sec_layer_info[idx].layer_address = (u32)vaddr;
...
{
	return drm_atomic_helper_update_plane(
		  plane, crtc, fb, crtc_x, crtc_y, crtc_w, crtc_h,
...
DEFINE_DRM_GEM_DMA_FOPS(ameba_fops);
