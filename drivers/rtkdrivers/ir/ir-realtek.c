...
static void rtk_ir_reg_dump(struct rtk_ir_dev *ir_rtk)
{
#if RTK_IR_REG_DUMP
...
static void rtk_ir_init_hw(struct rtk_ir_dev *ir_rtk)
{
...
static void rtk_ir_struct_init(
	struct rtk_ir_dev *ir_rtk,
	struct device_node *np)
{
...
static void rtk_ir_interrupt_config(
...
static u32 rtk_ir_get_int_status(struct rtk_ir_dev *ir_rtk)
{
	u32 status;
...
static u32 rtk_ir_get_mask_type(struct rtk_ir_dev *ir_rtk)
{
	u32 mask;
...
static void rtk_ir_clear_int_pending_bit(
...
static void rtk_ir_clear_rx_fifo(struct rtk_ir_dev *ir_rtk)
{
	rtk_ir_writel(ir_rtk->base, IR_RX_INT_CLR, IR_BIT_RX_FIFO_CLR);
}

static void rtk_ir_start_manual_rx_trigger(
...
static void rtk_ir_set_rx_cnt_threshlod(
	struct rtk_ir_dev *ir_rtk,
...
static void rtk_ir_clear_tx_fifo(struct rtk_ir_dev *ir_rtk)
{
...
static void ir_rx_prepare(struct rtk_ir_dev *ir_rtk)
{
	/* Initialize IR */
	ir_rtk->ir_param.ir_mode = IR_MODE_RX;
...
static void ir_recv_end(struct rtk_ir_dev *ir_rtk)
{
	int i, us_duration = 0;
...
static int ir_rx_recv(void *input_param)
{
	u32 reg_data;
	u32 data;
	u32 len, i;
...
static void ir_tx_start(struct rtk_ir_dev *ir_rtk)
{
	u32 tx_count = 0;
	int retry_count = 0;
...
static int ir_rtk_set_tx_duty_cycle(
...
static void ir_rtk_remove(struct platform_device *pdev)
{
