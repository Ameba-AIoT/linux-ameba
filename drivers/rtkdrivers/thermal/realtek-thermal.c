...
static int realtek_trip_walk_cb(struct thermal_trip *trip, void *arg)
{
	struct realtek_thermal_data *thermal = arg;
...
static int realtek_thermal_probe(struct platform_device *pdev)
{
	...
	/* set temperatures */
	thermal_zone_for_each_trip(thermal->th_dev, realtek_trip_walk_cb, thermal);
...
static void realtek_thermal_remove(struct platform_device *pdev)
{
