...
{
	struct rxi312_spi *spi = spi_controller_get_devdata(mem->spi->controller);
...
{
	struct rxi312_spi *spi = spi_controller_get_devdata(sdev->controller);
	struct rxi312_spi_map *map = spi->user_base;
	u32 ctrl0;
	u32 valid_cmd;
...
static int rxi312_spi_probe(struct platform_device *pdev)
{
	...
	host->setup = rxi312_spi_setup;
	host->mem_ops = &rxi312_spi_mem_ops;
	host->dev.of_node = pdev->dev.of_node;

	spi_controller_set_devdata(host, spi);
	...
	if (status) {
		dev_err(&pdev->dev, "Failed to register SPI host: %d\n", status);
		goto fail_register_host;
	}
	return status;
...
static void rxi312_spi_remove(struct platform_device *pdev)
{
	struct rxi312_spi *spi;
	struct resource *mem;
...
static struct platform_driver rxi312_spi_driver = {
	.driver = {
		...
		.of_match_table = rxi312_spi_of_match_table,
	},
