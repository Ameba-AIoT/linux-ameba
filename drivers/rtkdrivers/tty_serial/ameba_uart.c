...
extern void ameba_console_putchar(struct uart_port *port, unsigned char ch);
 ...
 * output given char
 */
void ameba_console_putchar(struct uart_port *port, unsigned char ch)
{
	wait_for_xmitr(port);
	ameba_uart_writel(port, ch, AMEBA_LOGUART_THR4);
}
...
static void ameba_uart_remove(struct platform_device *pdev)
{
